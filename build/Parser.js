...
function Parser(HTMLStr, ctx, ENTIDPRE='') {
    // 生成一个dom数
	...
	function addEntId(doms, ENTIDPRE=''){
		doms.forEach((item, index) => {
			// 如果是文本元素就特殊标记下
			if(!item.tag){
				item.entId = ENTIDPRE + index + '-text'
			}else{
				item.entId = ENTIDPRE + index
			}

			if(item.children){
				addEntId(item.children, ENTIDPRE + index + '-' )
			}
		})
	}

	addEntId(Dom, ENTIDPRE)
	console.log('domTree:', Dom);
    return Dom
}
